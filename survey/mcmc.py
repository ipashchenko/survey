...
def summary_stat(sample, bslns_sthrs_statuses, bsls_borderes, cred_mass=0.25):
    """
    Function that returns summary statistics for given data.
    :param data:
    :return:
    """
    fr_list = get_detection_fractions_in_baseline_ranges(bslns_sthrs_statuses,
                                                         bsls_borderes,
                                                         cred_mass=cred_mass)
    ...
    for i, bsls_s_thrs in enumerate(bsls_s_thrs_partitioned):
        ...
        det_fr = observe_sample(sample, baselines, pa, s_thrs)
...
def create_sample(p, size):
    """
    Create sample os sources (sizes, full fluxes & axis ratios) with size =
    ``size`` using parameters of distributions, specified in ``p``.
    :param p:
    ...
    """
    alpha_e = 5.
    mu_logs, std_logs, mu_loga, std_loga, beta_e = p
...
def indicator(p, summary, eps):
    """
    Function that returns log of prior density for parameters ``p`` if simulated
    model for them lies within tolerance level ``eps`` of summary statistics
    for data.
    """
    if p[1] <= 0:
        result = float("-inf")
    else:
        sim_data = np.random.normal(loc=p[0], scale=p[1], size=len(data))
        summary_data = summary(data)
        summary_sim = summary(sim_data)
        if (abs(summary_sim[0] - summary_data[0]) < eps[0]) and  (abs(summary_sim[1] - summary_data[1]) < eps[1]):
            result = 0
        else:
            result = float("-inf")
